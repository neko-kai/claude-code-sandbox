...
- **Restricted Write Access**: Only allows writing to:
  ...
  - Claude, Gemini & Codex configuration (`~/.claude`, `~/.gemini`, `~/.codex`)
...
Find out which rules to add by watching sandbox denials with:

```bash
/usr/bin/log stream --style compact --predicate 'eventMessage CONTAINS "Sandbox:" AND eventMessage CONTAINS "deny"'
```

Or by running `Console.app` and filtering errors by 'sandbox'
